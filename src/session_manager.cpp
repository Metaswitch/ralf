...
std::string SessionManager::create_opaque_data(Message* msg)
{
  ...
  return "{\"event\": {\"Accounting-Record-Type\": \"INTERIM\"}}";
}
  ...
  if (accepted)
  {
...

    {
      ...
      _timer_conn->send_put(msg->timer_id,
                            std::to_string(interim_interval),
                            std::to_string(msg->session_refresh_time),
                            "/call-id/"+msg->call_id,
    ...
    else if (msg->record_type.compare("START") == 0)
    {
      ...
      {
        _timer_conn->send_post(msg->timer_id,  // Chronos returns a timer ID which is filled in to this parameter
                               std::to_string(interim_interval), // interval
                               std::to_string(msg->session_refresh_time), // repeat-for
                               "/call-id/"+msg->call_id,
                               create_opaque_data(msg),
                               fake_trail);
      };
